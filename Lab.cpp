        ...
        if(isa<BinaryOperator>(inst)){
          ...
          if(isConstant(op1, Lconst)){
            if(isConstant(op2,Rconst )){
              ...
              {
                Value* where = nextSI->getPointerOperand();
                StoreInst* SI = new StoreInst(val, where);
                next->eraseFromParent();
                llvm::errs() << where << "\n";
                inst->insertAfter(SI);
              }
              //BasicBlock::iterator ii(inst);
            //  ReplaceInstWithValue(inst->getParent()->getInstList(), ii, val);
              //if(I.use_empty())
                //deadBin.insert(inst);
            }
          }
